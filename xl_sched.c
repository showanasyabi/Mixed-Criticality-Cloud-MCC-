...
static int sched_credit_domain_output(int domid)
{
    ...
    if (domid < 0) {//mcc
        printf("%-33s %4s %6s %4s %4s %4s \n", "Name", "ID", "wcet1 ", "wcet2", "period" , "criticality level" ); //mcc
        return 0;
    }
...
int main_sched_credit(int argc, char **argv)
{
    ...
    int weight = 5000, cap = 10000, mcc_period = 50000 , mcs_cri_level = 1; // mcc weight=wcet1 cap=wcet2
    int tslice = 0, ratelimit = 0;
    bool opt_w = false, opt_c = false,  opt_mcc_y = false, opt_mcc_o  = false; //mcc
    ...
    static struct option opts[] = {
        ...
        {"ratelimit_us", 1, 0, 'r'},
        {"cpupool", 1, 0, 'p'},
        {"mcc_period", 1, 0, 'y'},  // mcc
        {"mcc_cri_level", 1, 0, 'o'}, // mcc
        COMMON_LONG_OPTS
    };


    SWITCH_FOREACH_OPT(opt, "d:w:c:p:t:s:y:o:", opts, "sched-credit", 0) { // mcc
    case 'd':
        dom = optarg;
        break;
    case 'w':
    ...
    case 'p':
        ...
        break;
            ...
            case 'y': //mcc
                mcc_period = strtol(optarg, NULL, 10);
                opt_mcc_y = true;
                break;

            case 'o': //mcc
                mcs_cri_level = strtol(optarg, NULL, 10);
                opt_mcc_o = true;
                break;
    }
    ...
    if (opt_s) {
        ...
        if (!opt_w && !opt_c && !opt_mcc_y && !opt_mcc_o)// mcc { /* output credit scheduler info */
        ...
        // mcc
        if (opt_mcc_y)
            scinfo.period = mcc_period;
        if (opt_mcc_o)
            scinfo.budget = mcs_cri_level;
