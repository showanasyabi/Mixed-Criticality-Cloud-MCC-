...
int main_tmem_set(int argc, char **argv)
{
    ...
    uint32_t period = 0, cl = 0; // mcc
    int opt_w = 0, opt_c = 0, opt_p = 0;
    int opt_per = 0, opt_cl = 0; //  mcc
    int all = 0;
    int opt;
    int rc = 0;
    ...
    if (!opt_w && !opt_c && !opt_p && !opt_per && !opt_cl) { //mcc
    ...
    if (opt_per)
        libxl_tmem_set(ctx, domid, "period", period); //mcc

    if (opt_cl)
        libxl_tmem_set(ctx, domid, "cl", cl); // mcc
